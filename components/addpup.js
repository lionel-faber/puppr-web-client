...
export default function AddPup() {
    ...
    const onFinish = async (pet) => {
        ...
        try {
            const response = await fetch("http://localhost:5000/api/pets/add", requestOptions);
    ...
    return (
        <Form form={form} {...layout} onFinish={onFinish}>
            ...
            <Form.Item name="gender" label="Gender" rules={[{ required: true, message: "Please choose a gender" }]}>
                <Select>
                    <Select.Option value="male">male</Select.Option>
                    <Select.Option value="female">female</Select.Option>
                </Select>
            </Form.Item>
            <Form.Item name="age" label="Age" rules={[{ type: 'number', required: true, min: 0, max: 99, message: "Please enter the age" }]}>
                <InputNumber />
            </Form.Item>
            <Form.Item name="about" label="Introduction" rules={[{ required: true, message: "A few words would be nice" }]}>
                <Input.TextArea />
            </Form.Item>
            <Form.Item
                name="petPhoto"
